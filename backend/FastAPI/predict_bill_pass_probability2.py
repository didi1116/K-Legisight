...
# Prefer the data directory local to this module (backend/FastAPI/data/processed)
TRAIN_PKL = os.path.join(os.path.dirname(os.path.abspath(__file__)), "data", "processed", "bill_training_table.pkl")
# Backwards-compatible fallback (in case an alternate layout is used)
if not os.path.exists(TRAIN_PKL):
    alt = os.path.join(BASE_DIR, "data", "processed", "bill_training_table.pkl")
...
def predict_bill_pass_probability(query_text: str) -> Dict:
    ...
    if legislative_gap < 0.08:
        gap_level = "입법 결과가 논의 흐름과 거의 일치하는 사례"   # 논의와 결과가 거의 일치
    elif legislative_gap < 0.18:
        gap_level = "논의 방향과 결과가 대체로 일치하는 사례"     # 약한 불일치
    elif legislative_gap < 0.30:
        gap_level = "논의와 결과 사이에 유의미한 괴리가 존재하는 사례"     # 의미 있는 괴리
    elif legislative_gap < 0.45:
        gap_level = "논의 흐름과 결과가 구조적으로 불일치하는 사례"     # 구조적 괴리
    else:
        gap_level = "논의와 정반대의 결과 도출된 사례"   # 심각한 입법 괴리
    ...
    # --------------------------------------------------
    # (1) 과거 성과 요약
    passed_count = sum(e["label"] for e in evidence)
    total_count = len(evidence)
    historical_pass_rate = passed_count / total_count if total_count > 0 else 0

    # (2) 논의 분위기 설명
    if avg_coop > 0.1:
        discussion_desc = "전반적으로 협력적인 논의가 우세했습니다"
    elif avg_coop < -0.1:
        discussion_desc = "전반적으로 비협력적인 논의가 우세했습니다"
    else:
        discussion_desc = "논의는 중립적이거나 혼재된 양상을 보였습니다"

    # (3) 발언 충분성 해석
    if speech_confidence > 0.7:
        speech_desc = "발언 수가 충분하여 논의 신뢰도가 높았습니다"
    elif speech_confidence > 0.4:
        speech_desc = "일정 수준의 논의는 있었으나 충분하다고 보기는 어렵습니다"
    else:
        speech_desc = "발언 수가 적어 논의 기반 판단의 신뢰도가 낮습니다"

    # (4) 최종 확률 해석 문장
    if pass_prob >= 0.7:
        prob_desc = "과거 사례 기준으로 볼 때 가결 가능성이 높은 유형의 법안입니다"
    elif pass_prob >= 0.4:
        prob_desc = "가결과 부결 가능성이 혼재된 유형의 법안입니다"
    else:
        prob_desc = "과거 사례 기준으로 볼 때 가결 가능성이 낮은 유형의 법안입니다"
    ...
    explanation = (
        f"입력된 법안 '{query_text}'은(는) 과거 구조적으로 유사한 "
        f"{total_count}건의 법안을 기준으로 분석되었습니다. "
        f"이들 중 실제로 통과된 법안은 {passed_count}건으로, "
        f"과거 통과 비율은 {historical_pass_rate:.1%} 수준이었습니다. "
        f"{discussion_desc}. 또한 {speech_desc}. "
        f"이러한 요소들을 종합한 결과, "
        f"{prob_desc}. "
